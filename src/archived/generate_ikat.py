...
if __name__ == "__main__":
    ...
    if 'flatten' in args.model_path.lower():
        from data import DataCollatorForFunctionFlatten
        ...
        data_collator = DataCollatorForFunctionFlatten(
                ...
                n_statements=10 if (args.select_ptkb_as_conversation or args.all_ptkb_as_conversation) else 0,
    ...
    if 'compressed' in args.model_path.lower():
        from data import DataCollatorForFunctionCompressed
    ...
    if 'sep' in args.model_path.lower():
        # [TODO] replace data with data if it does better
        from data2 import DataCollatorForFunctionFlatten
        model = FiDT5_flat.from_pretrained(args.model_path)
        data_collator = DataCollatorForFunctionFlatten(
                tokenizer=tokenizer, 
                max_src_length=args.max_src_length,
    ...
    if 'castorini' in args.model_path.lower():
        from data import DataCollatorForNTR
    ...
    # Data # already the list
    dataset = get_ikat_dataset(args.input_file)

    ## post-process for ikat
    ### selected ptkbs
